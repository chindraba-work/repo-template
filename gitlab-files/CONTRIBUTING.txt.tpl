...
own when writing your commit messages. However, the Command Line version of git
makes doing so simple, use the "--signoff" or "-s" option when you make your
commit. Inside the commit message it will look like this:
...
in a manner verifiable by GitLab. Again, the git command makes this simple with
the "--gpg-sign[=<keyid>]" or "-S[<keyid>]" options for a commit. This, of
course, means that you will need to have a PGP key for the same email address as
you use for creating the commits. GitLab has a good page, "Signing commits with
GPG" (see below), which explains, and guides you through the process of creating
one. That page also helps you associate the new PGP key with your account on
GitLab, if you wish. I would recommend that where the instructions suggest
selecting the default of "RSA and RSA" that you select "ECC and ECC" instead.
The size of the keys are shorter and the time and work you computer uses to
generate and use the keys is less. Selecting the ECC algorithm causes the
selection of key size to be replaced with a selection of which elliptic curve to
use, in which case I recomment the "Curve 25519" option. The choice of which
option to use is a personal choice, and either way is acceptable here, as long
as the commits are signed with a valid PGP key.

OpenPGP public key
--------------------------------------------------------------------------------

One extra step that is required beyond the GitLab help example is that your key
also needs to be on a public key server somewhere which is accessable to myself,
and anyone else who wishes to verify signatures from this repo. Two choices,
that I know of, are the regular public key server network and Keybase (see
below). The public key server pool can be accessed directly by GNU Privacy
...
open an Issue on the project page, (see link below). Typos also qualify as an
issue. Maybe it is not a bug, or is the result of a prior decision, maybe it is
a problem and needs to be fixed. Either way the Issue will be seen and handled.

Find one, patch one
--------------------------------------------------------------------------------

If you find a truly serious bug, and know how to fix it, code the solution and
create a Merge request on GitLab, (see link below).
...
planned without taking it out of its planned scope. Asking, or offereing, won't
...
<https://pgp.mit.edu>
    ...
    New Issues on GitLab
<https://gitlab.com/<<GITLAB_NAMESPACE>>/<<GITLAB_PROJECT>>/-/issues/new>

    New Merge Requests on GitLab
<https://gitlab.com/<<GITLAB_NAMESPACE>>/<<GITLAB_PROJECT>>/-/merge_requests/new>
